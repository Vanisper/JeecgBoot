...
const execPromise = (command: string): Promise<{ success: boolean; data: string | null }> => {
...
const getInfo = (data: string): CommitInfo | null => {
  ...
  if (!matchGroups || matchGroups.subject === undefined || matchGroups.type === undefined) return null;
...
const main = async () => {
  // 获取远程仓库地址
  const { success, data } = await execPromise('git remote get-url --all origin');
  ...
  const replacePull = (content: string) => {
    return content.replace(/pull request (#(\d+))/, (_, data, number) => `pull request [${data}](${target}pull/${number})`);
  };
  ...
  const mdContent = res.map((item) => {
    ...
    if (item.id) {
      if (!item.pre?.tag) {
        ...
        md = `## [${item.tag}](${target}compare/${item.pre?.tag}...${item.tag}) (${days(item.date).format('YYYY-MM-DD')})`;
      }
    }

    const getFormatContent = (item: Log) => {
      return `* **${item.scope || 'all'}:** ${item.subject}([${item.sha}](${target}commit/${item.sha})) by@${item.author}\n`;
    };
    ...
    /** 一些修复 */
    const fixData = item.content.filter((item) => item.type === 'fix').map((item) => getFormatContent(item));

    /** 代码重构 */
    const refactorData = item.content.filter((item) => item.type === 'refactor').map((item) => getFormatContent(item));

    /** 性能优化 */
    const perfData = item.content.filter((item) => item.type === 'perf').map((item) => getFormatContent(item));

    /** 文档变更 */
    const docsData = item.content.filter((item) => item.type === 'docs').map((item) => getFormatContent(item));
    ...
    const content =
      ...
      '\n\n' +
      (docsData.length ? `### 📚 Documentation Update\n\n${docsData.join('')}` : '') +
      '\n\n';
